...
describe "faraday" do
  ...
  describe "middleware" do
    ...
    it "accepts a sha384 digest signatures" do # default
      require 'ey-hmac/faraday'
      Bundler.require(:rack)

      app = lambda do |env|
        authenticated = Ey::Hmac.authenticated?(env, accept_digests: [:sha384], adapter: Ey::Hmac::Adapter::Rack) do |auth_id|
          (auth_id == key_id) && key_secret
        end
        [(authenticated ? 200 : 401), {"Content-Type" => "text/plain"}, []]
      end
      ...
      connection = Faraday.new do |c|
        c.use :hmac, key_id, key_secret, sign_with: :sha384
        c.adapter(:rack, app)
      end
